...
function ItemCount({ onAdd, stock, productId }) {
  const { calcularCantidad, isInCart } = useContext(CartContext);
  ...
  return (
    <>
      ...
      <br />
        ...
        <Link to={'/cart'}>
          <button className={`${ isInCart(productId) ? "buy-btn" : "buy-btn-disabled"}`}>
